...
eat = [list(map(int, input().split())) for _ in range(d)]  # [사람 번호, 치즈 번호, 시간]
sick = [list(map(int, input().split())) for _ in range(s)]  # [사람 번호, 아픈 시간]

spoil_chz = set()

# t초까지 반복하면서, 치즈를 배열에 기록함
for t in range(1, 101):
    # t시간에 아팠다면, 그 사람이 t시간 전까지 어떤 치즈를 먹었는지 기록
    for i in range(s):
        if sick[i][1] == t:
            for j in range(51):
                if arr[sick[i][0]][j] == 1:
                    spoil_chz.add(j)  # 해당 치즈는 상했다고 가정
...
for i in range(51):
    for j in spoil_chz:
        if arr[i][j] == 1:
            ans += 1
            break  # 하나라도 감염된 치즈를 먹었으면 더 확인할 필요 없음
